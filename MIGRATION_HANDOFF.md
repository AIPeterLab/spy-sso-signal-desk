...
- Default branch: `main`
- Migration backup branch: `agent/add-volatility-adjusted-sso-strategy` (synchronized into `main` on 2026-09-08)
- The synchronization retained the newer generated signal, cycle, and dashboard files from `main` while bringing in the migration documentation and branch-only research artifacts.
- The test suite passes: 12 tests on 2026-09-08.

For recovery:

```powershell
git clone https://github.com/AIPeterLab/spy-sso-signal-desk.git
...
1. Verify the Business workspace has the required Codex access and reconnect any plugins or account-specific instructions that are still needed.
2. Back up or document the external Cloudflare Worker in its own repository/project.
